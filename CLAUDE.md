...
- **On-demand refresh only**: `ensure_auth_async()` is called by `garmin_api_call` at the start of every user-triggered Garmin request. There is NO periodic refresh loop — Render free sleeps the container after 15 min idle, so a long-interval loop never fires reliably. `proactive_token_refresh()` runs once at startup but routes through `attempt_refresh()` which respects the persistent backoff, so a cold-start during a Garmin cooldown is a no-op.
- Token health endpoints (JWT-auth): `GET /api/garmin/auth/token-health` (status), `POST /api/garmin/auth/refresh?force=true` (manual trigger; `force=true` bypasses backoff — use only when you're sure Garmin has released the account-level block), `POST /api/garmin/auth/inject-tokens` (manual fresh bundle, also clears DB backoff). Public: `GET /api/garmin/auth/status` (sanitized state from DB + last 10 bootstrap-log lines — no JWT needed, safe for cron and uptime monitors). The 503 from sync endpoints carries `Retry-After` header.
- Token regen recipe (when stuck in backoff): use `garth.resume(old_dir) + garth.client.refresh_oauth2()` from your local venv — connectapi.garmin.com often accepts refresh on older tokens with a wider grace period than the nominal 30d. Then base64 the bundle and POST to `/auth/inject-tokens` which clears the DB backoff and resumes.
...
- **Garmin auth backoff** (`_BACKOFF_LADDER_SECONDS` in `token_store.py`): 30 min → 2h → 6h → 24h → 24h. Persisted in `garmin_token_store` DB table so it survives Render container restarts. Account-level 429 from Garmin lasts 12-48h and each retry resets that timer, so backoff MUST be in hours.
